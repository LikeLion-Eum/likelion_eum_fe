...
function OfficeThumb({ office }) {
    // 서비스 레이어에서 mainPhotoUrl → thumbnailUrl 보정하지만, 방어적으로 둘 다 확인
    const src = office.thumbnailUrl ?? office.mainPhotoUrl ?? null;
    if (!src)
        return _jsx(Thumb, { alt: `${office.name} 썸네일` });
    return (_jsx("img", { src: src, alt: `${office.name} 썸네일`, className: "h-36 w-full rounded-lg object-cover", loading: "lazy", decoding: "async" }));
}
/* 인재 카드 */
