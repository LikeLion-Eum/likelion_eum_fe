...
export default function ProgramList() {
  ...
  // 백엔드 sort 규격: "field,dir" (필요 시 여러 개 지원)
  const sortParam = useMemo<string[]>(() => {
    switch (sort) {
      case "deadlineDesc":
        return ["receiptEndDate,desc"];
      case "titleAsc":
        return ["title,asc"];
      default:
        return ["receiptEndDate,asc"]; // deadlineAsc
    }
  }, [sort]);

  // recruiting 문자열 → boolean | undefined 로 매핑
  const recruitingParam = useMemo<boolean | undefined>(() => {
    if (recruiting === "open") return true;
    if (recruiting === "closed") return false;
    return undefined; // all
  }, [recruiting]);

  const toList = (d: any) => (Array.isArray(d) ? d : (d?.content ?? []));
  const toTotalPages = (d: any) => (typeof d?.totalPages === "number" ? d.totalPages : 0);
  ...
  async function load(p = page) {
    try {
      ...
      const useSearch =
        (q && q.trim().length > 0) ||
        recruitingParam !== undefined ||
        !!sortParam?.length;

      let data: any;

      if (useSearch) {
        // 검색/필터/정렬이 있을 때는 searchPrograms 사용
        const qTrim = q.trim();
        const params: Record<string, unknown> = {
          page: p,
          size: PAGE_SIZE,      // ✅ 명세: size
          sort: sortParam,      // 배열 반복 전송
        };
        if (recruitingParam !== undefined) params.recruiting = recruitingParam;
        if (qTrim) params.q = qTrim; // string만 넣음

        const res = await searchPrograms(params as {
          page: number;
          size: number;
          sort?: string[]; // 서버 규격에 맞춰 필요 시 수정
          recruiting?: boolean;
          q?: string;
        });
        data = res;
      } else {
        // 단순 목록은 fetchProgramsList(page, size)만
        const res = await fetchProgramsList(p, PAGE_SIZE);
        data = res;
      }

      setPrograms(toList(data));
      setTotalPages(toTotalPages(data));
    } catch (e: any) {
      setErr(
        String(e?.response?.data?.error ?? e?.message ?? "목록을 불러오지 못했습니다.")
      );
      setPrograms([]);
      setTotalPages(0);
    } finally {
        ...
        <div className="flex flex-col items-start justify-between gap-3 md:flex-row md:items-center">
          <div>
            ...
            <p className="muted mt-1 text-sm">
              키워드/모집여부/정렬을 설정해 빠르게 찾아보세요. ({headerHint})
            </p>
          </div>
        </div>
...
function Pagination({
  ...
  return (
    <div className="mt-4 flex justify-center gap-2">
      ...
      {nums.map((n, i) =>
        n === "…" ? (
          <span key={`e-${i}`} className="px-2 text-sm text-[var(--c-muted)]">
            …
          </span>
        ) : (
          <button
            ...
            className={`rounded-lg px-3 py-1 text-sm ${
              n === page ? "bg-[var(--c-brand)] text-white" : "border border-[var(--c-card-border)] bg-white"
            }`}
          >
            {n}
          </button>
