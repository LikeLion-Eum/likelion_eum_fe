...
export type RegionCascadeValue = { si: string; gu: string };

/** "서울 강남구" / "충남 아산시" / "" 로 합치기 */
export function joinLocation({ si, gu }: RegionCascadeValue) {
...
export default function RegionCascadeSelect({ value, onChange, label = "지역", className }: Props) {
  const { si, gu } = value;
  ...
  return (
    <div className={`flex items-center gap-2 ${className || ""}`}>
      ...
      <select
        value={si}
        onChange={(e) => onChange({ si: e.target.value, gu: "" })}
      ...
      <select
        value={gu}
        onChange={(e) => onChange({ si, gu: e.target.value })}
        className="h-10 rounded-lg border border-[var(--c-card-border)] bg-white px-3 text-sm"
        disabled={!si}
      >
        <option value="">{si ? "전체" : "시/도를 먼저 선택"}</option>
        {guOptions
          .filter((g) => !!g)
          .map((g) => (
